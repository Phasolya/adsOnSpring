...
public class MailAddressDAOImpl implements MailAddressDAO {
    ...
    public List<String> findAllSuitableEmails(Advertisement ads) {
        ...
        Query query = manager.createQuery("SELECT ma.user.email.mailAddress FROM MatchingAd ma WHERE ma.category.id = :categoryId " +
                "AND ma.priceFrom <= :asdPrice AND ma.priceTo >= :asdPrice " +
                "AND ma.title LIKE :title", String.class);
